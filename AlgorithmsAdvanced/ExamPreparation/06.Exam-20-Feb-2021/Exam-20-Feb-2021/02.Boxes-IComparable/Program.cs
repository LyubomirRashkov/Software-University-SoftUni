...
namespace _02.Boxes
{
	public class Box : IComparable<Box>
	{
		...
		public int CompareTo(Box other)
		{
			...
			int depthComparison = this.Depth.CompareTo(other.Depth);

			int heightComparison = this.Height.CompareTo(other.Height);

			return Math.Min(widthComparison, Math.Min(depthComparison, heightComparison));
		}
	...
	public class Program
	{
		...
		private static void FillArrays(List<Box> boxes, int[] lengths, int[] previousOnes, ref int indexOfLastAddedNumber)
		{
			...
			for (int i = 0; i < boxes.Count; i++)
			{
				...
				for (int j = 0; j < i; j++)
				{
					if (boxes[i].CompareTo(boxes[j]) > 0 && lengths[j] + 1 > currentLength)
					{
